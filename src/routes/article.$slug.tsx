...
import { getArticle, getAuthor, articles } from "@/data/articles";
        ...
        {author && (
          <div className="mt-6 flex items-center gap-3 border-y border-border py-4">
            <Link
              to="/author/$id"
              params={{ id: author.id }}
              className="flex h-12 w-12 items-center justify-center rounded-full bg-brand font-bold text-brand-foreground"
            >
              {author.avatar}
            </Link>
            <div>
              ...
              <div className="text-sm text-muted-foreground">
                {article.publishedAt} • {article.readTime} पढ़ने का समय
              </div>
            </div>
          </div>
        )}
