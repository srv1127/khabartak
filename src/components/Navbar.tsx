...
const links = [
  ...
  { to: "/submit-video", label: "वीडियो सबमिट" },
  { to: "/write", label: "ब्लॉग लिखें" },
];
    ...
    <header className="sticky top-0 z-50 border-b border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/80">
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-4">
        ...
        <nav className="hidden items-center gap-1 md:flex">
          ...
          <Link
            to="/login"
            className="ml-2 rounded-md bg-brand px-4 py-2 text-sm font-bold text-brand-foreground transition-opacity hover:opacity-90"
          >
            लॉगिन
          </Link>
        </nav>
        ...
        <nav className="border-t border-border bg-background md:hidden">
          <div className="flex flex-col p-2">
            ...
            <Link
              to="/login"
              onClick={() => setOpen(false)}
              className="mt-2 rounded-md bg-brand px-3 py-3 text-center text-base font-bold text-brand-foreground"
            >
              लॉगिन
            </Link>
          </div>
